...
helm install --name hbase \
  ...
  --set image.repository=192.168.2.30/kubernetes/hbase \
...
helm install --name opentsdb \
  --set opentsdb.replicas=3 \
  --set image.tag=2.4.0 \
  --set image.repository=192.168.2.30/kubernetes/opentsdb \
  --set opentsdb.zookeeper.zk_quorum=zk-0.zk-svc.default.svc.cluster.local,zk-1.zk-svc.default.svc.cluster.local,zk-2.zk-svc.default.svc.cluster.local \
  opentsdb
